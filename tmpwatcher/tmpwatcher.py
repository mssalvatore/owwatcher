...
class TMPWatcher:
    ...
    def __init__(
        self, perms_mask, file_archiver_builder, logger, alerter, is_snap=False
    ):
    ...
    def _watch_for_world_writable_files(self, watch_dir, recursive):
        self.logger.info(
            "Setting up inotify watches on %s and its subdirectories" % watch_dir
        )
        ...
        if self.is_snap:
            ...
            self.logger.debug(
                "It was detected that this application is"
        ...
        while True:
            try:
                ...
                msg = (
                    "Caught error while adding initial inotify watches on tree [%s]: %s"
                    % (watch_dir, str(fnf))
                )
                ...
                self.logger.warning(
                    "Caught inotify error (%s). Rebuilding inotify watchers..."
                    % str(iex)
                )
                ...
                self.logger.error(
                    "Caught unexpected error (%s). Rebuilding inotify watchers..."
                    % str(ex)
                )
    ...
    def _process_event(self, watch_dir, event, file_archiver):
        ...
        if self._should_send_alert(event_path, filename):
            file_archiver.add_event_to_archive_file_queue(
    ...
    def _should_send_alert(self, event_path, filename):
        try:
            ...
            self.logger.debug(
                ...
                % (full_path, "{:03o}".format(self.perms_mask))
            )

            status = os.stat(full_path)
            self.logger.debug(
                "Permissions of %s are %s"
                % (full_path, "{:03o}".format(status.st_mode))
            )
